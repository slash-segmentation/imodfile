...
# is peachy.
#
@test "Test imod source script does not exist" {

  # verify $KEPLER_SH is in path if not skip this test
  skipIfKeplerNotInPath
 
  echo "0,,imodinfo: Error (-1) reading imod model. ($THE_TMP/foo/data/some.mod)," > "$THE_TMP/bin/command.tasks"

  mkdir -p "$THE_TMP/foo/data"
  echo "hi" > "$THE_TMP/foo/data/some.mod"
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -imodfile "$THE_TMP/foo" -imodSourceScript $THE_TMP/noexist -imodInfoCmd "$THE_TMP/bin/command" -CWS_outputdir $THE_TMP $WF
  ...
  [ "$status" -eq 0 ]
  [ "${lines[0]}" == "simple.error.message=Error examining imod file" ]
  [[ "${lines[1]}" == "detailed.error.message=Error from imodinfo : "* ]]
