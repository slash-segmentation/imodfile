...
# is peachy.
#
@test "Test where no imod file exists" {

  # verify $KEPLER_SH is in path if not skip this test
  skipIfKeplerNotInPath

  mkdir -p "$THE_TMP/data"
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -imodfile "$THE_TMP" -CWS_outputdir $THE_TMP $WF
  ...
  [ "$status" -eq 0 ]
  [ "${lines[0]}" == "simple.error.message=Unable to find IMOD file" ]
  [ "${lines[1]}" == "detailed.error.message=No file .imod or .mod extension found in $THE_TMP" ]
